...
Figure \ref{ch3_fig_L200_surface_background} shows the {\Ltwo} physics spectrum. Muon and multiplicity cuts remove backgrounds such as muons and gamma rays, and the events left are primarily surface events in gray background. As shown in green, Pulse Shape Discrimination (PSD) cuts are highly effective in removing them, but the unknown efficiency of those cuts introduces uncertainty in the background model. This is because the spectrum shape of the alphas before the cuts is unknown.
...
Signal formation in Germanium is well understood via the Shockley-Ramo theorem. This enables the building of accurate simulations that can model the waveforms. Simulations within the LEGEND Collaboration include {\siggen} and \texttt{SSD} simulations. 
